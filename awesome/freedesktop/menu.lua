...
function menu.build(args)
    ...
    menu_gen.generate(function(entries)
        ...
        for k, v in pairs(menu_gen.all_categories) do
            table.insert(result, { k, {}, menu_utils.lookup_icon(v.icon_name) or menu_utils.lookup_icon(string.lower(k)) })
        end
            ...
            for _, cat in pairs(result) do
                if cat[1] == v.category then
                    if not menu.has_value(skip_items, v.name) then
                        table.insert(cat[2], { v.name, v.cmdline, v.icon or menu_utils.lookup_icon(string.lower(v.name))})
                    end
                    break
                end
            end
